...
# Install curl, git, sqlite3, nginx, passenger, imagemagick, nodejs, redis
RUN apt-get update && apt-get install -y --force-yes \
    build-essential curl git zlib1g-dev libssl-dev libreadline-dev \
    libyaml-dev libxml2-dev libxslt-dev libsqlite3-dev sqlite3 \
    libxml2-dev libxslt1-dev libcurl4-openssl-dev python-software-properties \
    libffi-dev apt-transport-https nginx-full passenger libpq-dev imagemagick \
    nodejs redis-server language-pack-en && apt-get clean

# Add keys to verify install packages
RUN gpg --keyserver keyserver.ubuntu.com --recv-keys 561F9B9CAC40B2F7 && gpg --armor --export 561F9B9CAC40B2F7 | apt-key add -

# Configure passenger in pkg sources
...
# Set acess and perms for pkg sources file
...
# Setup postgres user for postgres
USER postgres
RUN /etc/init.d/postgresql start \
    && psql --command "CREATE USER pguser WITH SUPERUSER PASSWORD 'pguser';" \
    && createdb -O pguser pgdb

# Setup root user for postgres
USER root
...
ADD ./start_services /usr/bin/start_services
